...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (iMsg)
	{
		...
		pIMyMath -> lpVtbl -> SumOfTwoIntegers(pIMyMath, iNum1, iNum2, &IMyMath);

		// display the result
		wsprintf(str, TEXT("Sum of %d and %d = %d"), iNum1, iNum2, IMyMath);
		...
		pIMyMath ->  lpVtbl -> SubtractionOfTwoIntegers(pIMyMath, iNum1, iNum2, &iSub);
...
void ComErrorDescriptionString(HWND hwnd, HRESULT hr)
{
	...
	if(FormatMessageW(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL, hr, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPWSTR)&szErrorMessage, 0, NULL) != 0)
	{
		...
		// _s 		= secure
		// %#		= HexaDecimal Value
		wsprintf(str, TEXT("%08X : %s"), hr, szErrorMessage );
		...
		wsprintf(str, TEXT("[Could not find description for error # %#x.] \n"), hr);
